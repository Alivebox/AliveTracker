 ...
 */
Ext.define('AliveTracker.view.projects.calendar.util.WeekEventRenderer', {
        ...
        render: function(o) {
            ...
            for (; w < weekCount; w++) {
                if (!grid[w] || grid[w].length == 0) {
                    ...
                    dt = AliveTracker.view.projects.calendar.util.Date.add(dt, {days: 7});
                } else {
                    var row,
                        d = 0,
                        wk = grid[w],
                        startOfWeek = Ext.Date.clone(dt),
                        endOfWeek = AliveTracker.view.projects.calendar.util.Date.add(startOfWeek, {days: dayCount, millis: -1});
                        ...
                        if (wk[d]) {
                            ...
                            for (; ev < ct; ev++) {
                                ...
                                if (!evt.isSpan || evt.isSpanStart) {
                                    ...
                                    item._weekIndex = w;
                                    item._renderAsAllDay = item[AliveTracker.view.projects.calendar.data.EventMappings.IsAllDay.name] || evt.isSpanStart;
                                    item.spanLeft = item[AliveTracker.view.projects.calendar.data.EventMappings.StartDate.name].getTime() < startOfWeek.getTime();
                                    item.spanRight = item[AliveTracker.view.projects.calendar.data.EventMappings.EndDate.name].getTime() > endOfWeek.getTime();
                                    ...
                                    var diff = AliveTracker.view.projects.calendar.util.Date.diffDays(dt, item[AliveTracker.view.projects.calendar.data.EventMappings.EndDate.name]) + 1,
