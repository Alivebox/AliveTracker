 ...
 */
Ext.define('AliveTracker.view.projects.calendar.view.MonthDayDetail', {
    extend: 'Ext.Component',
    alias: 'widget.monthdaydetailview',

    requires: [
        'Ext.XTemplate',
        'AliveTracker.view.projects.calendar.view.AbstractCalendar'
    ],
    ...
    refresh: function() {
        ...
        evts = this.store.queryBy(function(rec) {
            var thisDt = Ext.Date.clearTime(this.date, true).getTime(),
                recStart = Ext.Date.clearTime(rec.data[AliveTracker.view.projects.calendar.data.EventMappings.StartDate.name], true).getTime(),
                startsOnDate = (thisDt == recStart),
                spansDate = false;

            if (!startsOnDate) {
                var recEnd = Ext.Date.clearTime(rec.data[AliveTracker.view.projects.calendar.data.EventMappings.EndDate.name], true).getTime();
        ...
        evts.each(function(evt) {
            ...
            item._renderAsAllDay = item[M.IsAllDay.name] || AliveTracker.view.projects.calendar.util.Date.diffDays(item[M.StartDate.name], item[M.EndDate.name]) > 0;
            item.spanLeft = AliveTracker.view.projects.calendar.util.Date.diffDays(item[M.StartDate.name], this.date) > 0;
            item.spanRight = AliveTracker.view.projects.calendar.util.Date.diffDays(this.date, item[M.EndDate.name]) > 0;
